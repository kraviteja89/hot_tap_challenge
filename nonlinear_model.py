...
"""
def cp(u1, u2):
        val = (cp1 + cp2*(1/(1 + exp(-cpa*(u1-u_melt))) - 1/(1 + exp(-cpa*(u2-u_melt))))/(u1-u2)
               if ne(u1, u2) else cp1)
"""
...
solution_file = File(result_path + '//nonlinear_solution.pvd')
solution_file << (u_n, 0)
...
cp = Expression('abs(u_n-u_n1)>0 ? '
                'cp1 + cp2*(1/(1 + exp(-cpa*(u_n-u_melt))) - 1/(1 + exp(-cpa*(u_n1-u_melt))))/(u_n-u_n1)'
                ':cp1', degree=2, u_n=u_n, u_n1=u_n1, u_melt=u_melt, cp1=cp1, cp2=cp2, cpa=cpa)

a = 0.5 * axsym * rho * cp * u * v * dx + axsym * dt * k * dot(grad(u), grad(v)) * dx + \
    axsym * dt * h_amb * u * v * ds(3) + axsym * dt * h_process * u * v * ds(2)

L = 0.5 * axsym * rho * cp * u_n1 * v * dx + axsym * dt * f * v * dx(3) + \
...
for n in range(tsteps):
    ...
    if (n+1) % output_steps == 0:
        ...
        plt.tight_layout()
        plt.savefig(result_path + "//nonlinear_model%0*d.png" % (4, im_number), dpi=96)
        im_number += 1
        plt.close()